...
/** Auth states a `NiteOwlUserButton` link can be visible in. */
export type NiteOwlUserButtonLinkVisibility =
	| "authenticated"
	| "unauthenticated"
	| "always";

/** A simple link entry rendered as a `DropdownMenuItem` in the `NiteOwlUserButton` menu. */
export type NiteOwlUserButtonLink = {
	/** Visible label. */
	label: ReactNode;
	/** Destination URL. */
	href: string;
	 ...
	 * @default "always"
	 */
	visibility?: NiteOwlUserButtonLinkVisibility;
};

export type NiteOwlUserButtonProps = {
	className?: string;
 ...
 */
export function NiteOwlUserButton({
	className,
	align,
	sideOffset,
	size = "default",
				...
				{size === "icon" ? (
					...
					<>
						{session || sessionPending || settingActiveSession ? (
							<NiteOwlUserView isPending={!!settingActiveSession} />
						) : (
							<>
								<NiteOwlUserAvatar />
