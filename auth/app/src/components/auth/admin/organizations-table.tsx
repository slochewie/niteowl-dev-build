...
type CreatedFilter = "all" | "24h" | "7d" | "30d" | "90d";
...
export function OrganizationsTable({
	...
	const activeFilterCount =
		Number(createdFilter !== "all") +
		Number(memberFilter !== "all") +
	...
	const filteredOrganizations = useMemo(() => {
		...
		const result = organizations.filter((organization) => {
			...
			const createdAt = new Date(organization.createdAt).getTime();

			let matchesCreated = true;

			switch (createdFilter) {
				case "24h":
					matchesCreated = createdAt >= now - 24 * 60 * 60 * 1000;
					break;

				case "7d":
					matchesCreated = createdAt >= now - 7 * 24 * 60 * 60 * 1000;
					break;

				case "30d":
					matchesCreated = createdAt >= now - 30 * 24 * 60 * 60 * 1000;
					break;

				case "90d":
					matchesCreated = createdAt >= now - 90 * 24 * 60 * 60 * 1000;
					break;
			}
			...
			if (dateRange?.from) {
				...
				matchesDateRange = matchesDateRange && createdAt >= from.getTime();
			}
			...
			if (dateRange?.to) {
				...
				matchesDateRange = matchesDateRange && createdAt <= to.getTime();
			}
			...
			switch (memberFilter) {
				case "zero":
					matchesMembers = organization.memberCount === 0;
					break;

				case "one":
					matchesMembers = organization.memberCount === 1;
					break;
				...
				case "more-than-five":
					matchesMembers = organization.memberCount > 5;
					break;

				case "more-than-ten":
					matchesMembers = organization.memberCount > 10;
					break;
			}

			return (
				matchesSearch && matchesCreated && matchesDateRange && matchesMembers
			);
		});
	...
	}, [organizations, search, sortMode, createdFilter, memberFilter, dateRange]);

	const pageCount = Math.max(
		1,
		Math.ceil(filteredOrganizations.length / rowsPerPage),
	);
	...
	const showingFrom = filteredOrganizations.length === 0 ? 0 : startIndex + 1;
	...
	return (
		<div className="flex flex-col gap-5">
			<div className="text-sm text-muted-foreground">
				Showing {showingFrom}
				{"-"}
				{showingTo} of {filteredOrganizations.length}{" "}
				...
				<div className="flex flex-1 flex-col gap-2 sm:flex-row">
					...
					<DropdownMenu>
						...
						<DropdownMenuContent align="start">
							...
							<DropdownMenuItem onClick={() => setSortMode("members-desc")}>
								Members (Most)
							</DropdownMenuItem>

							<DropdownMenuItem onClick={() => setSortMode("members-asc")}>
								Members (Fewest)
							</DropdownMenuItem>
						</DropdownMenuContent>
					</DropdownMenu>
				...
				<div className="flex items-center gap-2">
					<Popover>
						...
						<PopoverContent align="end" className="w-[280px] p-0">
							<div className="px-4 py-3 font-medium">Filter Organizations</div>
							...
							<div className="space-y-3 p-4">
								...
								<Select
									value={createdFilter}
									onValueChange={(value) => {
										setCreatedFilter(value as CreatedFilter);
							...
							<div className="space-y-3 p-4">
								...
								>
									...
									<SelectContent>
										...
										<SelectItem value="zero">0 members (Abandoned)</SelectItem>

										<SelectItem value="one">1 member</SelectItem>
