...
const reconcileSourceBodySchema =
  z.object({
    sourceId:
      z.string().min(1)
  })

function ldapUsername(
  username:
    string | null,
  email: string
) {
...
function sourceBaseDn(
  slug: string
) {
  return (
    `ou=${slug},dc=niteowl,dc=dev`
  )
}

function sourceRuntimeSchema(
  slug: string
) {
  return (
    "glauth_" +
        ...
        createAuthEndpoint(
          ...
          async (ctx) => {
            ...
            try {
              ...
              if (!runtimePort) {
                throw new Error(
                  "Unable to allocate GLAuth runtime port"
                )
              }

              await provisionRuntimeSchema(
                client,
                runtimeSchema
              )
