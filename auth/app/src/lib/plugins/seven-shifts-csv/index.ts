...
const createCsvSourceBodySchema =
  z.object({
    name:
      z.string()
        .trim()
...
const renameCsvSourceBodySchema =
  z.object({
    sourceId:
      z.string().min(1),
    name:
...
async function csvSourceExists(
  pool: Pool,
  sourceId: string
) {
  const result =
...
function getCsvSourceDirectory(
  storageRoot: string,
  sourceId: string
) {
  return join(
...
async function readSelectedFileId(
  directory: string
) {
  try {
    const value =
...
async function writeSelectedFileId(
  directory: string,
  fileId: string
) {
  await writeFile(
...
async function getTargetLocations({
   ...
   */
  const result =
    await pool.query<TargetLocation>(
      `
        SELECT
          l.id,
          l."organizationId",
...
export const sevenShiftsCsv = ({
  pool,
  storageRoot
}: SevenShiftsCsvOptions) =>
  ({
    id: "seven-shifts-csv",
            ...
            try {
              ...
              if (
                ...
                return ctx.json(
                  {
                    error:
                      `A CSV Source named "${name}" already exists`
                  },
                  {
                    status:
                      409
          ...
          async (ctx) => {
            ...
            if (
              ...
              return ctx.json(
                {
                  error:
                    "CSV file exceeds the 10 MB upload limit"
                },
                {
                  status:
                    413
        ...
        createAuthEndpoint(
          ...
          async (ctx) => {
            ...
            if (
              ...
              return ctx.json(
                {
                  error:
                    "Selected CSV file was not found"
                },
                {
                  status:
                    404
            ...
            for (
              ...
              if (
                ...
                return ctx.json(
                  {
                    error:
                      `No mapped 7shifts role "${roleName}" exists for location "${location.name}"`
                  },
                  {
                    status:
                      400
          ...
          if (
            ...
            for (
              ...
              const assignmentKey =
                `${employee.employeeId}\u0000${location.id}\u0000${role.id}`
            ...
            if (
              ...
              const result =
                await pool.query<ExistingAssignment>(
                  `
                    SELECT
                      id,
                      "employeeId",
