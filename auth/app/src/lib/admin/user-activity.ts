...
import { requireAdminRead } from "@/lib/admin/access";
...
export const getAdminUserActivity = createServerFn({
	method: "GET",
})
	.validator(
		(data: { userId: string; limit?: number; offset?: number }) => data,
	)
	.handler(async ({ data }): Promise<AdminUserActivityResult> => {
		await requireAdminRead();

		const apiKey = process.env.BETTER_AUTH_API_KEY;

		const limit = Math.min(Math.max(data.limit ?? 100, 1), 100);
		...
		if (!apiKey) {
			return {
				...
				limit,
				offset,
				error: "BETTER_AUTH_API_KEY is not configured",
			};
		}
		...
		const organizationIds = organizationsResult.rows
		...
		const url = new URL(`${getDashApiUrl()}/events/activity`);
		...
		try {
			...
			if (!response.ok) {
				...
				return {
					...
					limit,
					offset,
					error: body || `Unable to load activity: HTTP ${response.status}`,
				};
			}
