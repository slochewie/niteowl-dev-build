import { P as Plugin, a as PluginKey, N as NodeSelection, T as TextSelection, S as Selection, b as SelectionRange } from "./prosemirror-state.mjs";
...
import { T as Transform } from "./prosemirror-transform.mjs";
let readFromCache;
let addToCache;
...
function moveColumn(moveColParams) {
  ...
  const newTable = moveTableColumn$1(table.node, indexesOriginColumn, indexesTargetColumn);
...
function moveRow(moveRowParams) {
  ...
  const newTable = moveTableRow$1(table.node, indexesOriginRow, indexesTargetRow);
  tr.replaceWith(table.pos, table.pos + table.node.nodeSize, newTable);
  if (!select) return true;
  const map = TableMap.get(newTable);
  const start = table.start;
...
export {
  ...
  TableMap as T,
  tableNodes as a,
  moveTableColumn as b,
  deleteColumn as c,
  deleteTable as d,
  deleteRow as e,
  findTable as f,
  goToNextCell as g,
  addColumnBefore as h,
  isInTable as i,
  addColumnAfter as j,
  setCellAttr as k,
  columnResizing as l,
