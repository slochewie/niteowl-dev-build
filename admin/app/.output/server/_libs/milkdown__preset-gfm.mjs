...
import { m as markRule, g as findParentNodeType, h as cloneTr, i as findParentNodeClosestToPos } from "./milkdown__prose.mjs";
import { f as $inputRule, l as $pasteRule, b as $command, d as $nodeSchema, i as $markAttr, j as $markSchema, k as $useKeymap, a as $prose, c as $remark } from "./milkdown__utils.mjs";
import { a as listItemSchema, p as paragraphSchema } from "./milkdown__preset-commonmark.mjs";
import { i as expectDomTypeError } from "./milkdown__exception.mjs";
import { i as imeSpan } from "./prosemirror-safari-ime-span.mjs";
import { P as Plugin, a as PluginKey, S as Selection, T as TextSelection } from "./prosemirror-state.mjs";
import { t as tableEditing, a as tableNodes, g as goToNextCell, i as isInTable, m as moveTableRow, b as moveTableColumn, C as CellSelection, d as deleteTable, c as deleteColumn, e as deleteRow, h as addColumnBefore, j as addColumnAfter, s as selectedRect, k as setCellAttr, l as columnResizing, T as TableMap, f as findTable } from "./prosemirror-tables.mjs";
import { t as toggleMark } from "./prosemirror-commands.mjs";
import { I as InputRule } from "./prosemirror-inputrules.mjs";
import { S as Slice, F as Fragment } from "./prosemirror-model.mjs";
import { r as remarkGfm } from "./remark-gfm.mjs";
...
var strikethroughInputRule = $inputRule((ctx) => {
  return markRule(new RegExp("(?<![\\w:/])(~{1,2})(.+?)\\1(?!\\w|\\/)"), strikethroughSchema.type(ctx));
});
...
var tablePasteRule = $pasteRule((ctx) => ({ run: (slice, _view, isPlainText) => {
  ...
  function fixTable(node) {
    ...
    if (rowsCount >= 3) {
      ...
      return node.type.create(node.attrs, [newHeaderRow2, ...remainingRows]);
    }
    const headerCells = Array(colsCount).fill(0).map(() => tableHeaderSchema.type(ctx).createAndFill());
  ...
  function cleanEmptyParagraphs(fragment2) {
    const nodes = [];
    const allNodes = [];
    fragment2.forEach((node) => allNodes.push(node));
...
export {
  selectRowCommand as a,
  moveRowCommand as b,
  setAlignCommand as c,
  deleteSelectedCellsCommand as d,
  addColAfterCommand as e,
  addColBeforeCommand as f,
  addRowAfterCommand as g,
  addRowBeforeCommand as h,
  gfm as i,
  toggleStrikethroughCommand as j,
  strikethroughSchema as k,
  createTable as l,
  moveColCommand as m,
  selectColCommand as s,
  tableSchema as t
};
