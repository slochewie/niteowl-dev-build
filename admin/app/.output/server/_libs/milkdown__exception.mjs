...
function aiProviderError(cause) {
  const message = cause instanceof Error ? cause.message : safeStringify(cause);
  return new MilkdownError(ErrorCode.aiProviderError, `AI provider error: ${message}`, { cause });
}
function aiBuildContextError(cause) {
  const message = cause instanceof Error ? cause.message : safeStringify(cause);
  return new MilkdownError(ErrorCode.aiBuildContextError, `AI buildContext failed: ${message}`, { cause });
}
export {
  contextNotFound as a,
  createNodeInParserFail as b,
  ctxCallOutOfScope as c,
  serializerMatchError as d,
  callCommandBeforeEditorView as e,
  docTypeError as f,
  getAtomFromSchemaFail as g,
  missingMarkInSchema as h,
  expectDomTypeError as i,
  aiBuildContextError as j,
  aiProviderError as k,
  missingNodeInSchema as m,
  parserMatchError as p,
  stackOverFlow as s,
  timerNotFound as t
};
